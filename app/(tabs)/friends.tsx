...
import {
  ...
  subscribeToFriends, subscribeToOutgoingRequests, subscribeToPendingRequests,
} from '@/lib/firestore';
      ...
      <Modal visible={addModalVisible} transparent animationType="slide" onRequestClose={() => setAddModalVisible(false)}>
        <KeyboardAvoidingView style={styles.modalBg} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <View style={styles.modalSheet}>
            ...
            {showResults ? (
              <>
                <Text style={styles.resultLabel}>{searchResults.length === 0 ? 'No users found' : `${searchResults.length} user${searchResults.length !== 1 ? 's' : ''} found`}</Text>
